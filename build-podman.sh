...
echo sudo podman build --build-arg OS_RELEASE=${OS_RELEASE} --build-arg OS_IMAGE=${OS_IMAGE} --build-arg DEVBUILD=${DEVBUILD} -t ${BUILD_ARCH}/zwave2mqtt:${BUILD_ID} -f Containerfile
sudo podman build --build-arg OS_RELEASE=${OS_RELEASE} --build-arg OS_IMAGE=${OS_IMAGE} --build-arg DEVBUILD=${DEVBUILD} -t ${BUILD_ARCH}/zwave2mqtt:${BUILD_ID} -f Containerfile
