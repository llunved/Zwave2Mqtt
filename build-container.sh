...
echo sudo podman build --build-arg FEDORA_RELEASE=${FEDORA_RELEASE} --build-arg FEDORA_IMAGE_BUILD=${FEDORA_IMAGE_BUILD} --build-arg FEDORA_IMAGE_RT=${FEDORA_IMAGE_RT} -t llunved/zwave2mqtt:${BUILD_ID} -f Dockerfile
