...
OS_IMAGE=${OS_IMAGE:-"registry.fedoraproject.org/fedora"}
BUILD_ID=${BUILD_ID:-`date +%s`}
BUILD_ARCH=${BUILD_ARCH:-`uname -m`}
PUSHREG=${PUSHREG:-""}

echo sudo podman build --build-arg OS_RELEASE=${OS_RELEASE} --build-arg OS_IMAGE=${OS_IMAGE} -t ${BUILD_ARCH}/zwave2mqtt:${BUILD_ID} -f Dockerfile.Fedora
sudo podman build --build-arg OS_RELEASE=${OS_RELEASE} --build-arg OS_IMAGE=${OS_IMAGE} -t ${BUILD_ARCH}/zwave2mqtt:${BUILD_ID} -f Dockerfile.Fedora
